...
class GenCode():
    ...
    def declaration(self, tree, file_name):
        ...
        for decl in declaractions:
            if decl.name == 'declaracao_funcao':
                ...
                print('inicialização de variaveis')
        self.saveCode(file_name)
    ...
    def saveCode(self, file_name):
        # Cria o diretório 'resultadosLlvm' se ele não existir
        output_dir = "resultadosLlvm"
        os.makedirs(output_dir, exist_ok=True)
        
        # Cria o nome do arquivo de saída
        base_name = os.path.basename(file_name)  # Obtém apenas o nome do arquivo, sem o caminho
        llvm_file_name = os.path.join(output_dir, f"{base_name}.ll")  # Adiciona a extensão '.ll'
        
        # Salva o código no arquivo .ll
        with open(llvm_file_name, 'w') as file:
